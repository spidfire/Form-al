	...
	function validateInput($data,ElementBase $element){
		if(is_array($data)){
			if(count($data) == $this->exactCount)
				...
				$element->error("Verkeerde hoeveelheid" , "De invoer moet bestaan uit  exact {$this->exactCount} elementen");		
		}else
			$element->error("Te weinig inhoud", "De invoer moet bestaan uit  exact {$this->exactCount} elementen");
		return false;
	}
