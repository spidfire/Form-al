...
class IconUpload extends Input{
	function setValue($value){
		...
		}elseif(preg_match("/colorchange/",$value)){
    ...
    }elseif(preg_match("/flaticons/",$value)){
      $this->value =  "typeicon";
      $this->extend_fields['iconVal']->setValue($value);
		}elseif(!empty($value)){
	...
	function getValue(){
		...
		if ($icondetails['selected'] == "typeicon") {
      	...
      	}else if ($icondetails['selected'] == "typeimage") {
      		return $icondetails['imageupload'];
      	}else if ($icondetails['selected'] == "fontawesome") {
	...
	function render(){
		...
		$inp = $e->addHTML('
        	<div class="icondiv">
                ...
                {% if menu.iconVal is empty %}
                <img id="image" style="height:50px; width:50px; background-color: #D1D1D1;" src="{{ROOT}}template\img\no-available-image.png" alt="Image not found">
                {% else %}
                <img id="image" style="height:50px; cursor:pointer; background-color: #D1D1D1;" id="img_{{menu.id}}" src="{{menu.iconVal}}" alt="image" onClick="window.open(document.getElementById(\'img_{{menu.id}}\').src);">
                {% endif %}
                </a>
                '. $this->extend_fields['iconVal']->render() .'
                <br><br>
                <p class="help-block">
                    {{ "Select an image"|trans }} {{ menu.iconVal!="" ? "; However, this will replace the existing icon"|trans : "" }}
                </p>
                ...
                '
             	<div id="inline1" style="width:800px;max-height: 800;background-color: #D1D1D1;display: none;">
             		...
             		box-shadow: none;
             		border-radius: none;
             		" placeholder="{{"Search"|trans}}" onkeyup="showImagesByAlt()">
             		<br><br>
                   	{% for img in images %}
                   		<img src="http://m.almanapp.nl/flat/flaticons/{{ img }}" alt="{{img}}" style="width: 100px;height: 100px;display: inline-block; padding: 10px;" onclick="saveUrl(\'{{ img }}\')">
                   	{% endfor %}
               </div>
            </div>
